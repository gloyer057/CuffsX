    ...
    public void onInitialize() {
        ...
        net.fabricmc.fabric.api.event.player.UseEntityCallback.EVENT.register((player, world, hand, entity, hitResult) -> {
                ...
                if (!CuffManager.isCuffed(applier) && CuffManager.isCuffed(target)) {
                    if (LeashManager.isLeashed(target.getUuid())) {
                        ...
                        if (holderUuid != null && holderUuid.equals(applier.getUuid())) {
                            ...
                            // Обновляем lockedPos на текущую позицию — чтобы не телепортировало обратно
                            ...
                            applier.sendMessage(Text.literal("§c" + target.getName().getString() + " уже на поводке у другого игрока."), true);
                        }
                    } else {
            ...
            if (applier.getMainHandStack().getItem() instanceof org.gloyer057.cuffsx.item.HandcuffsItem hi) {
                // Если только что сняли — не надеваем сразу
                if (CuffManager.justRemoved(applier.getUuid())) return ActionResult.SUCCESS;

                // Проверяем флаг enabled до того как забирать наручники
                if (!CuffManager.isEnabled()) {
                    applier.sendMessage(Text.literal("§cНаручники нельзя одеть на игрока в данный момент."), false);
                    return ActionResult.SUCCESS;
                }

                // Проверяем приват FTB Chunks ДО того как забираем наручники
                if (ClaimChecker.isClaimed(applier, applier.getBlockPos())) {
                    applier.sendMessage(Text.literal("§cНельзя надеть наручники в чужом привате."), false);
                    return ActionResult.SUCCESS;
                }

                // Если у цели уже есть хоть одни наручники — надеваем вторые сразу без таймера
                if (CuffManager.isCuffed(target)) {
                    applier.getMainHandStack().decrement(1);
                    applier.playerScreenHandler.syncState();
                    CuffManager.applyCuffsImmediate(applier, target, hi.getCuffType());
                    return ActionResult.SUCCESS;
                }

                // Забираем наручники сразу, потом запускаем прогресс
                applier.getMainHandStack().decrement(1);
                applier.playerScreenHandler.syncState();
