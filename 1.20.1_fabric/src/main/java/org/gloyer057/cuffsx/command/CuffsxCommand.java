...
public class CuffsxCommand {
            ...
            literal("cuffsx")
                ...
                .then(literal("log")
                    ...
                    .executes(ctx -> {
                        ...
                        if (recent.isEmpty()) {
                            source.sendFeedback(
                                () -> Text.literal("Нет взаимодействий за последние 24 часа."), false);
                        } else {
                    ...
                    .then(argument("player", StringArgumentType.word())
                        .executes(ctx -> {
                            ...
                            if (onlineTarget != null) {
                                // Игрок онлайн — снимаем через стандартный путь
                            ...
                            // Игрок офлайн — ищем по имени в CuffState через UserCache
                            ...
                            // Снимаем офлайн — только убираем из state и логируем
                            for (CuffType type : CuffType.values()) {
                                if (state.hasCuff(offlineUuid, type)) {
                                    // Находим запись для лога
                                    ...
                                    // Возвращаем предмет исполнителю
                                    ItemStack newStack = new ItemStack(
                                            type == CuffType.HANDS ? ModItems.HANDCUFFS_HANDS : ModItems.HANDCUFFS_LEGS);
                                    try {
                                        ServerPlayerEntity executor = source.getPlayer();
    ...
    /**
     * Ищет UUID офлайн-игрока через UserCache сервера.
     */
