...
public class CuffApplies {
    ...
    public record ApplyEntry(
            long timestamp,       // оригинальное время надевания
            long transferredAt,   // время переноса из лога в applies
            String applierName,
            String targetName,
            CuffType cuffType,
            Vec3d coords
    ...
    public static class AppliesState extends PersistentState {
        ...
        /** Удаляет записи старше 6 часов от времени переноса. */
        ...
        /** Удаляет все записи где targetName совпадает (игнорируя регистр). */
    ...
    /**
     * Переносит APPLY-запись из лога в applies.dat и уведомляет игроков с правом cuffsx.applies.
     */
    public static void transfer(CuffLog.LogEntry entry) {
        ...
        // Уведомление всем с правом cuffsx.applies
        ...
        for (ServerPlayerEntity player : currentServer.getPlayerManager().getPlayerList()) {
            if (Permissions.check(player, "cuffsx.applies", 4)) {
