...
public class CuffManager {
    ...
    public static boolean applyCuffs(ServerPlayerEntity applier, ServerPlayerEntity target, CuffType type) {
        ...
        if (!enabled) {
            applier.sendMessage(net.minecraft.text.Text.literal("Наручники нельзя одеть на игрока в данный момент."), false);
            return false;
        }
        if (ClaimChecker.isClaimed(applier, target.getBlockPos())) {
            applier.sendMessage(net.minecraft.text.Text.literal("§cНельзя надеть наручники — цель находится в привате."), false);
            return false;
        }
    ...
    public static CuffType removeCuffs(ServerPlayerEntity applier, ServerPlayerEntity target) {
        ...
        if (state.hasCuff(target.getUuid(), CuffType.HANDS)) typeToRemove = CuffType.HANDS;
        else if (state.hasCuff(target.getUuid(), CuffType.LEGS)) typeToRemove = CuffType.LEGS;
        else return null;
        CuffRecord removedRecord = state.getRecords(target.getUuid()).stream()
                .filter(r -> r.cuffType() == typeToRemove).findFirst().orElse(null);
        state.removeRecord(target.getUuid(), typeToRemove);
        ItemStack stack = new ItemStack(typeToRemove == CuffType.HANDS ? ModItems.HANDCUFFS_HANDS : ModItems.HANDCUFFS_LEGS);
        if (!applier.getInventory().insertStack(stack)) applier.dropItem(stack, false);
        if (removedRecord != null) CuffLog.log(CuffLog.Action.REMOVE, removedRecord);
    ...
    public static void sendHud(ServerPlayerEntity player) { sendHud(player, -1, -1); }
    public static void sendHud(ServerPlayerEntity player, int applyProgress) { sendHud(player, applyProgress, -1); }

    public static void sendHud(ServerPlayerEntity player, int applyProgress, int removeProgress) {
        ...
        if (type == CuffType.LEGS) {
            ...
            if (attr != null) {
                ...
                attr.addPersistentModifier(new EntityAttributeModifier(
                        LEGS_SPEED_MODIFIER_UUID, LEGS_SPEED_MODIFIER_NAME,
    ...
    public static void removeRestrictions(ServerPlayerEntity player, CuffType type) {
        if (type == CuffType.LEGS) {
            EntityAttributeInstance attr = player.getAttributeInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED);
            if (attr != null) attr.removeModifier(LEGS_SPEED_MODIFIER_UUID);
        }
    }
}
