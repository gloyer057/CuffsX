...
public class CuffManager {
    ...
    public static boolean applyCuffs(ServerPlayerEntity applier,
        ...
        // Проверка привата FTB Chunks — только позиция таргета
        if (ClaimChecker.isClaimed(applier, target.getBlockPos())) {
            applier.sendMessage(
                net.minecraft.text.Text.literal("§cНельзя надеть наручники — цель находится в привате."), false);
            return false;
        }
    ...
    public static boolean applyCuffsImmediate(ServerPlayerEntity applier,
        ...
        // Проверка привата — только позиция таргета
        if (ClaimChecker.isClaimed(applier, target.getBlockPos())) {
            applier.sendMessage(
                net.minecraft.text.Text.literal("§cНельзя надеть наручники — цель находится в привате."), false);
            return false;
    ...
    public static void sendHud(ServerPlayerEntity player, int applyProgress) {
        sendHud(player, applyProgress, -1);
    }
